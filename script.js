...
function detaGama(text) {
  ...
  let disSign = chuKy.join(", ");
  console.log("DS: " + disSign.toString());

  document.querySelector("#text-cky").innerHTML = `-δ:${disSign}\n-γ:${gama}`;
  document.querySelector("#deta").value = deta;
...
function kiemTraChuKy(text) {
  ...
  let deteCal = document.querySelector("#deta").value;

  let detaArr = deteCal.split(", "); // Mảng chứa các giá trị của Deta - Chữ ký
  let detaCheckVTArr = deta.split("");
  let detaCheckArr = detaCheck.split("");
  let arr_1 = [];
  let arr_2 = [];

  console.log("Deta arr: " + detaArr);

  // BĂM TEXT
  let bam = CryptoJS.MD5(text).toString(CryptoJS.enc.Hex); // 128 / 4 = 32
  console.log("Text băm: " + parseInt(bam, 16).toString(2));
  ...
  console.log("Hash[1]: " + parseInt(hash[1], 16).toString(2));
  ...
  console.log(detaCheckVTArr + " và " + detaCheckArr);
...
taoKhoa.addEventListener("click", () => {
  let soP = parseInt(Math.floor(Math.random() * 100000000));
  while (isSNT(soP) != true) {
    soP = Math.floor(Math.random() * 100000000);
  }
